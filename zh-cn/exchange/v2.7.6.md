...
- 数组中的的第四个对象，为转账金额，类型为bytearray，前面没加0x，按小端序处理后值为200000000。这里根据金额不同，会有两种类型，一种是integer类型，另一种是bytearray类型。交易所处理该数值时，应当特别注意，如果类型为integer，其数值转换方式与bytearray不同。
```json
{
  "type": "ByteArray",
  "value": "00c2eb0b"
